...
| `stage` | `String`, default `'uncertain'` | **Plain string, not a Postgres enum** — see §1 and §6. Validated at the application layer against the API's stage registry, the same way `NotificationDelivery.eventKey` is validated against `NOTIFICATION_EVENTS` rather than a database type. |
...
| `test_version_code` | `String` FK → `civics_test_versions.code` | **Resolved once, at orientation submit time**, from the filing date the learner enters — not recomputed live on every read. This is the rejected-alternatives §11 entry on why the cutoff-date logic lives in one place: E8's interview engine and E2's content both need to join against one settled value per learner, and a client-side or per-call-site recomputation of "did they file before or after 20 Oct 2025" is a second place that logic can drift from the first. |
...
- **`interview_countdown`** — Title: "*N* days until your interview" (a real,
  ...
  never a placeholder). Reason: "Steady practice between now and then
  matters more than a single big push." Button: "Go to Practice."
...
| **A Postgres enum for `learner_profiles.stage`** | The same reasoning `NotificationDelivery.eventKey`'s comment gives: a stage's owning epic must land on its own schedule without an `ALTER TYPE` migration gating it, and a row written under today's eight values must still read back if one is ever renamed. §1, §3.2. |
