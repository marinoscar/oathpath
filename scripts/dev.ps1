<#
.SYNOPSIS
    OathPath Development Script for Windows

.DESCRIPTION
    Manages the OathPath development environment using Docker Compose.
...
if (-not (Test-Path $BaseCompose)) {
    ...
    Write-Err "Make sure you're running this script from the OathPath repository."
    exit 1
}

function Show-Help {
    Write-Host ""
    Write-Info "OathPath Development Script"
    Write-Host "====================================="
    Write-Host ""
...
function Start-Services {
    Write-Info "Starting OathPath services..."
    if ($Otel) {
...
function Stop-Services {
    Write-Info "Stopping OathPath services..."
    if ($Service) {
...
function Restart-Services {
    Write-Info "Restarting OathPath services..."
    if ($Service) {
...
function Rebuild-Services {
    Write-Info "Rebuilding OathPath services (no cache)..."
    if ($Otel) {
        Write-Info "Including OpenTelemetry observability stack..."
    }
    if ($Service) {
...
function Clean-Services {
    ...
    if ($confirmation -eq "yes") {
        Write-Info "Cleaning up OathPath services and volumes..."
