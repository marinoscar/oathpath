#!/usr/bin/env node
/**
 * Idempotent provisioning for the OathPath S3 bucket.
 *
 * Creates (if missing) and configures the bucket the API uploads files to:
 ...
 *
 * Usage. @aws-sdk/client-s3 is an API dependency, so the simplest place to run
